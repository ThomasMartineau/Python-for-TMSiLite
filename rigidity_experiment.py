...
class main_window(QMainWindow):
    ...
    def get_option(self):
        ...
        option['fsample'] =self.fsample.value()
        # Cue
    ...
    def get_condition(self):
        # condition
        condition = []
        cue_check = [self.High.isChecked(), self.Low.isChecked()]
        cue_label = ['High', 'Low']
        per_check = [box.isChecked() for box in [self.Baseline, self.CW, self.CCW, self.UNS]]
        per_label = ['Baseline', 'CW', 'CCW', 'Random']
        # cue the label
        k = 0
        for c, cl in zip(cue_check, cue_label):
            if c:
                for p, pl in zip(per_check, per_label):
                    if p:
                        k += 1
                        condition.append((cl, pl, k))
                        print((cl, pl, k))
        return condition
    ...
    def preview(self):
        ...
        list_block = [experiment.block(2, option, c) for c in condition] #if it is checked
    ...
    def generate(self):
        ...
        n = option['cue']
        #creat a block list
        list_block = option['repetition']*[experiment.block(n, option, c) for c in condition] #if it is checked             
        # random the option
        if option['randomise']:
        ...
        time = [traj['time'][-1] for traj in self.trajectory] 
        # time
